...
def render_mesh_selector(umap_df, mesh_df):
    ...
    if st.session_state.get("clear_lasso", False):
    ...
    for ev in events:
        ...
        if name in st.session_state.get('selected_faculty', []):
            st.session_state.selected_faculty.remove(name)
        else:
            st.session_state.get('selected_faculty', []).append(name)

    # ————————————————————————————————————————————————————————————————
    # 6) Determine which faculty set we’ll download
    sel_manual = st.session_state.get('selected_faculty', [])
    ...
    if sel:
        ...
        if st.button("Clear selections"):
            st.session_state.get('selected_faculty', []).clear()
